	...
	while (itr.hasNext()) {
		...
		if (!item.isFormField()) {
			...
			try {
				...
				ufile.setAddress(saveUrl + newFileName);
				ufile.setName(fileName);
				ufile.setSize(fileSize);
				ufile.setType(fileExt);
				ufile.setPri(0);
				ufile.setTime(new Date());
				UfileServiceImpl us = new UfileServiceImpl();
				us.AddUfile(ufile);
				ufile = us.GetUfileByAddress(ufile.getAddress());
			} catch (Exception e) {
				e.printStackTrace();
			...
			if (fileExt.contains("gif") 
				...
				obj.put("url", "/file/down/"+ ufile.getId());
